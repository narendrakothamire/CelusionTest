...
public class MainActivity extends AppCompatActivity {
    ...
    private AdapterView.OnItemClickListener listItemClickListener = new AdapterView.OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            long count = orderDao.queryBuilder().where(OrderDao.Properties.CustomerID.eq(selectedCutomer)).count();

            if (count > 0) {
                // customerOrders = orderDao.queryBuilder().where(OrderDao.Properties.CustomerID.eq(adapterCustomer.getItem(position).getCustomerID())).list();
            } else {
                which = 200;
                performNetwokCall();
            }
